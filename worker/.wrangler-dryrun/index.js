...
__name(toE164Phone, "toE164Phone");

// ../lib/calendly/server.ts
var CALENDLY_API_BASE = "https://api.calendly.com";
var CalendlyApiError = class extends Error {
  static {
    __name(this, "CalendlyApiError");
  }
...
function calendlyRouteStatus(error) {
...
__name(calendlyRouteStatus, "calendlyRouteStatus");
var cachedEventType = null;
function getCalendlyConfig() {
  ...
  return { token, eventTypeUri, locationKind: locationKind || void 0 };
}
__name(getCalendlyConfig, "getCalendlyConfig");
function formatCalendlyError(body, status) {
  if (typeof body !== "object" || body === null) {
    return `Calendly request failed (${status})`;
  }
  const record = body;
  const detail = record.details?.[0];
  const detailText = detail ? [detail.message, detail.parameter, detail.code].filter(Boolean).join(". ") : null;
  if (record.message && detailText) return `${record.message} (${detailText})`;
  if (record.message) return record.message;
  return `Calendly request failed (${status})`;
}
__name(formatCalendlyError, "formatCalendlyError");
async function calendlyRequest(config, path, init) {
  ...
  if (!response.ok) {
    ...
    const message = response.status === 429 ? "Please wait a few minutes and try again." : formatCalendlyError(body, response.status);
...
async function getEventTypeDetails(config) {
  if (cachedEventType?.uri === config.eventTypeUri) {
    return cachedEventType.details;
  }
  const data = await calendlyRequest(config, config.eventTypeUri.replace(CALENDLY_API_BASE, ""));
  const prepQuestion = data.resource?.custom_questions?.find(
    (item) => item.enabled !== false && item.name != null && typeof item.position === "number"
  ) ?? null;
  const details = {
    locationKind: data.resource?.locations?.[0]?.kind ?? null,
    prepQuestion: prepQuestion ? { question: prepQuestion.name, position: prepQuestion.position } : null
  };
  cachedEventType = { uri: config.eventTypeUri, details };
  return details;
}
__name(getEventTypeDetails, "getEventTypeDetails");
async function resolveLocationKind(config, eventType) {
  if (config.locationKind) return config.locationKind;
  return eventType.locationKind;
}
__name(resolveLocationKind, "resolveLocationKind");
async function listAvailableTimes(input) {
  ...
  const data = await calendlyRequest(config, `/event_type_available_times?${params.toString()}`);
  return (data.collection ?? []).filter((slot) => slot.status === "available" && slot.start_time).map((slot) => slot.start_time);
}
__name(listAvailableTimes, "listAvailableTimes");
async function createInvitee(input) {
  const config = getCalendlyConfig();
  if (!config) {
  ...
  const data = await calendlyRequest(config, "/invitees", {
    method: "POST",
...
__name(createInvitee, "createInvitee");
async function cancelScheduledEvent(eventUri, reason) {
  const config = getCalendlyConfig();
  if (!config) {
    throw new Error("Calendly is not configured");
  }
...
__name(cancelScheduledEvent, "cancelScheduledEvent");
...
// ../lib/calendly/validation.ts
var AVAILABILITY_MAX_RANGE_MS = 7 * 24 * 60 * 60 * 1e3;
var bookSchema = external_exports.object({
  ...
  timezone: external_exports.string().trim().min(1).max(80),
...
var cancelSchema = external_exports.object({
  eventUri: external_exports.string({ required_error: "Missing event" }).min(1, "Missing event").refine((value) => value.startsWith("https://api.calendly.com/scheduled_events/"), {
    message: "Invalid event"
  }),
  reason: external_exports.string().trim().max(1e3).optional()
});
function resolveAvailabilityRange(startParam, endParam) {
  const now = /* @__PURE__ */ new Date();
  const requestedStart = startParam ? new Date(startParam) : now;
  const requestedEnd = endParam ? new Date(endParam) : new Date(now.getTime() + AVAILABILITY_MAX_RANGE_MS);
  if (Number.isNaN(requestedStart.getTime()) || Number.isNaN(requestedEnd.getTime())) {
    return null;
  }
  const startTime = new Date(Math.max(requestedStart.getTime(), now.getTime() + 6e4));
...
__name(resolveAvailabilityRange, "resolveAvailabilityRange");
...
async function handleBook(request, env, cors) {
  ...
  let phone;
  try {
    phone = toE164Phone(parsed.data.phone) ?? void 0;
  } catch (error) {
...
async function handleCancel(request, cors) {
  const body = await request.json().catch(() => null);
