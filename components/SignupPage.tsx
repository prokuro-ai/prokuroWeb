...
import { normalizeEmail, signUp } from '@/lib/auth'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 'confirmSignUp') {
        setConfirmEmail(normalizeEmail(form.email))
        return
      }
      await finishSignUp()
    } catch (err) {
      ...
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          <h1 className="text-[24px] font-semibold text-[#0f1b2d]" style={{ letterSpacing: '-0.3px' }}>
            {confirmEmail ? 'Verify your email' : 'Start your free trial'}
          </h1>
          <p className="mt-2 text-[14px] text-[#7a8598]">
            {confirmEmail ? 'Check your inbox for a verification code.' : 'Upload your first BOM in 60 seconds.'}
          </p>
        </div>
