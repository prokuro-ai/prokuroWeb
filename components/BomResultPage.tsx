...
interface BomResultPageProps {
  id?: string
}

export default function BomResultPage({ id: propId }: BomResultPageProps) {
  const params = useParams<{ id: string }>()
  const [, navigate] = useLocation()
  const { user, loading: authLoading } = useAuth()
  const uploadId = propId ?? params.id ?? ''
  ...
  }, [authLoading, user, uploadId, navigate])
    ...
    <AppLayout>
      <div className="flex h-14 flex-shrink-0 items-center justify-between border-b border-[#d6deea] bg-white px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <DeleteBomButton bomId={uploadId} bomName={result.source_filename} onDeleted={() => navigate('/dashboard')} />
        </div>
      </div>
